/**
 * 本地文章样例数据（仅开发用途：无后台服务时的本地调试）。
 *
 * 线上文章一律来自后台 API（见 src/data/articlesApi.js，spec §7.6）；
 * 前台不再回退本地数据，防止已下架文章"复活"。原 OSS manifest 加载
 * 机制（VITE_ARTICLES_MANIFEST_URL）随迁移完成退役。
 */
...
/** 取指定 id 的文章，并附带上一篇/下一篇（循环）；仅开发调试用。 */
